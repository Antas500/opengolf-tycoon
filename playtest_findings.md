...
## Garden shed pass — September 25, 2026

- The separate "The garden shed" window is gone. The Improvements tab lists all 28 decorations as isometric tiles (`DecorationTileButton` / `DecorationTileArt`) in the same two interlocking rows the Course Terrain and Buildings tabs use; the shelf scrolls sideways and keeps the shed's category order.
- Pressing a tile starts placement straight away; price, upkeep, category, description and unlock state live in the rich hover popup. Locked ornaments stay on the shelf greyed out and unselectable, with the requirement in their tooltip, and unlock as rating, reputation or hole count is earned. `O` and Course Review's "garden" advice open the tab.
