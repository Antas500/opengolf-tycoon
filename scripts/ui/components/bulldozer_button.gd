...
func _apply_styles() -> void:
	var face := COLOR_FACE_HOVER if is_hovered() else COLOR_FACE
	if is_pressed() or (_is_active and is_hovered()):
		face = COLOR_FACE_PRESSED
	var ring := COLOR_RING_ACTIVE if _is_active else COLOR_RING
	var border := 3.0 if _is_active else 1.5
	for state in ["normal", "focus", "disabled"]:
		_style_for(state, face, ring, border)
	_style_for("hover", COLOR_FACE_HOVER, ring, border)
	_style_for("pressed", COLOR_FACE_PRESSED, ring, border)
	_style_for("hover_pressed", COLOR_FACE_PRESSED, ring, border)
