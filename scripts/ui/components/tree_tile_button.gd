...
func configure_tree(type: String, data: Dictionary) -> void:
	...
	var tree_cost: int = int(data.get("cost", 20))
	var desc: String = "Adds beauty and obstacles. Places a %s." % t_name.to_lower()
	configure("tree_" + type, t_name, "", "", desc, tree_cost, 0)
	tooltip_text = ""
	accessibility_description = "%s. Build $%d. %s" % [tool_name, tree_cost, desc]
...
func _get_sprite_path() -> String:
	...
	if tree_type == "pine" and season_name == "winter":
		var course_theme: int = CourseTheme.Type.PARKLAND
		if GameManager:
			course_theme = GameManager.current_theme
		if course_theme in TreeEntity.SNOWY_THEMES:
			var key := "pine_winter"
			if key in TreeEntity.SEASONAL_SPRITE_PATHS and ResourceLoader.exists(TreeEntity.SEASONAL_SPRITE_PATHS[key]):
				return TreeEntity.SEASONAL_SPRITE_PATHS[key]
